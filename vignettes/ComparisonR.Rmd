...
source("precomputed-vignette-cache.R")
comparison_cache <- load_precomputed_vignette_cache(
  "ComparisonR",
  cache_file = "ComparisonR.RDS",
  project_root = ".."
)
invisible(list2env(
  comparison_cache,
...
```{r begin-comparison-cache-regeneration, include = FALSE, eval = FALSE, purl = TRUE}
# From the project root, extract this vignette with knitr::purl(), then
# evaluate the extracted script from the vignettes directory.
pkgload::load_all("..", quiet = TRUE)
source("precomputed-vignette-cache.R")
comparison_cache_file <- "ComparisonR.RDS"
```
...
```{r fit-comparison-models, eval = FALSE, purl = TRUE} 
...
To compute the Bayes factor in favor of $\mathcal{H}_0$ over $\mathcal{H}_+$, I need to first obtain the marginal likelihoods. The only additional step is specifying an R function that returns the log likelihood of the model. Here, instead of writing out the loops, I take advantage of the fact that summation on the log scale equals to multiplication on the original scale.
...
```{r bridge-comparison-models, eval = FALSE, purl = TRUE}
...
`JAGS_bridgesampling()` returns each marginal likelihood on the natural-log scale. The log Bayes factor in favor of the null hypothesis is therefore the difference between the two log marginal likelihoods; exponentiating returns the Bayes factor on its ordinary ratio scale.

```{r}
log_BF_0plus <- marglik_model_H0$logml - marglik_model_Hp$logml
BF_0plus <- exp(log_BF_0plus)
BF_0plus
```
...
```{r save-comparison-cache, include = FALSE, eval = FALSE, purl = TRUE}
write_precomputed_vignette_cache(
  objects = list(
    ttest_model_H0 = ttest_model_H0,
    ttest_model_Hp = ttest_model_Hp,
    marglik_model_H0 = marglik_model_H0,
    marglik_model_Hp = marglik_model_Hp
  ),
  vignette = "ComparisonR",
  cache_file = comparison_cache_file,
  project_root = ".."
)
```
