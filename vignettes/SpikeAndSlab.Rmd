...
spike_and_slab_cache <- load_precomputed_vignette_cache(
  "SpikeAndSlab",
  cache_file = "SpikeAndSlab.RDS",
  project_root = ".."
)
invisible(list2env(
  spike_and_slab_cache,
...
```{r begin-spike-and-slab-cache-regeneration, include = FALSE, eval = FALSE, purl = TRUE}
...
source("precomputed-vignette-cache.R")
spike_and_slab_cache_file <- "SpikeAndSlab.RDS"
```
...
```{r fit-spike-and-slab-comparison-models, eval = FALSE, purl = TRUE} 
M0 <- JAGS_fit(
...
```{r bridge-spike-and-slab-comparison-models, eval = FALSE, purl = TRUE}
...
```{r fit-spike-and-slab-model, eval = FALSE, purl = TRUE}
MS <- JAGS_fit(
  model_syntax = model_likelihood,
  data         = list(y = y, N = N),
  prior_list   = list("sigma" = prior_sigma),
...
```{r save-spike-and-slab-cache, include = FALSE, eval = FALSE, purl = TRUE}
write_precomputed_vignette_cache(
  objects = list(
    M0 = M0,
    M1 = M1,
    marglik_model_H0 = marglik_model_H0,
    marglik_model_H1 = marglik_model_H1,
    MS = MS
  ),
  vignette = "SpikeAndSlab",
  cache_file = spike_and_slab_cache_file,
  project_root = ".."
)
```
