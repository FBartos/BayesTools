...
- restores literal `expression()` formula terms that reference sampled scalar or one-dimensional indexed parameters, persisting their data and parameter dependencies for draw-aware fitted/new-data prediction and marginal-likelihood reconstruction while rejecting opaque JAGS-derived nodes that cannot be replayed
